...
#
# This startup script can be used to start a condor head in a container
# 
#
...
if [ "${myuname}" == "root" ]; then
  ...
  (cd /opt/htcondor && ./condor_install --type=manager --owner=${HTCONDOR_USER} --local-dir=/var/lib/htcondor )
  if [ $? -ne 0 ]; then
  ...
  (cd /opt/htcondor && ./condor_install --type=manager --local-dir=/var/lib/htcondor )
  if [ $? -ne 0 ]; then
    echo "ERROR: condor_install failed. Aborting."
    exit 1
  fi
